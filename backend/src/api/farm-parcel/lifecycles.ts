...
function validateGeoJSONPolygon(geojson: unknown): {
    valid: boolean
    error?: string
    ...
    if (gj.type === 'Feature') {
        ...
        return {
            valid: false,
            error: 'boundary_geojson must be a GeoJSON Polygon or Feature with Polygon geometry',
        }
    }
    ...
    try {
        ...
        return {
            valid: false,
            error: `Failed to calculate area: ${err instanceof Error ? err.message : 'Unknown error'}`,
        }
    }
...
function validateLandStatus(status: unknown): {
    valid: boolean
    error?: string
} {
