...
export default factories.createCoreController('api::farm.farm', ({ strapi }) => ({
  /**
  ...
  async findWithSummary(ctx) {
    try {
      ...
      const results = await strapi.documents('api::farm.farm').findMany(queryParams);

      const resultsWithSummary = results.map((farm: Record<string, unknown>) => {
        const farmData = farm as Record<string, unknown>;
        const parcels = (farmData.farm_parcels as Array<Record<string, unknown>>) || [];
  ...
  async findOneWithSummary(ctx) {
    try {
      ...
      const queryParams = {
        ...
        populate: {
          farm_parcels: {
            fields: ['area_hectares', 'land_status', 'parcel_code'],
      ...
      const result = await strapi.documents('api::farm.farm').findOne({ documentId: id, ...queryParams });
