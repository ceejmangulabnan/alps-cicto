    ...
    async beforeCreate(event: { params: { data: Record<string, unknown> } }) {
        ...
        if (data.farmers) {
            const farmerIds = Array.isArray(data.farmers)
                ? data.farmers.map((f: unknown) =>
                      typeof f === 'object' && f !== null
                          ? (f as Record<string, unknown>).documentId ||
                            (f as Record<string, unknown>).id
                          : f
                  )
                : [
                      typeof data.farmers === 'object' && data.farmers !== null
                          ? (data.farmers as Record<string, unknown>)
                                .documentId ||
                            (data.farmers as Record<string, unknown>).id
                          : data.farmers,
                  ]
        ...
        if (data.barangay) {
            const barangayId =
                typeof data.barangay === 'object' && data.barangay !== null
                    ? (data.barangay as Record<string, unknown>).documentId ||
                      (data.barangay as Record<string, unknown>).id
                    : data.barangay
            ...
            if (barangayId) {
                ...
                data._barangayCode = (barangay as Record<string, unknown>)
                    .code as string
            }
        }
